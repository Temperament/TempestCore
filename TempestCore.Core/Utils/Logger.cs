...
namespace TempestCore.Core.Utils
{
    public class Logger : IDisposable
    {
        ...
        public virtual void InfoAuth(string format, params object[] str)
        {
            if (_isDisposed)
                return;
            var val = string.Format(format, str);
            var tmp = string.Format("{0} [AUTHSERVER]: {1}", DateTime.Now.ToString("HH:mm:ss dd.MM.yyyy"), val);
            var tmp2 = string.Format("[AUTHSERVER]: {0}", val);
            lock (_sync)
            {
                _stream.WriteLine(tmp);
                if (WriteToConsole)
